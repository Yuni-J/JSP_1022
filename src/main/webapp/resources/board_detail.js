...
function printCommentList(result){
	...
	for(let i=0; i<result.length; i++){
		let html = `<div>`;
		html += `<div>${result[i].cno}, ${result[i].bno}, ${result[i].writer}, ${result[i].regdate} `;
		html += `<div>`;
...
// 화면에서 데이터를 만들어서 보내느 방법 => post
// 데이터를 보낼때 method=post, headers(Content-Type), body를 작성해서 전송
// 서버에서 데이터를 주는 방법 => get
...
async function postCommentToServer(cmtData){
	try{
		...
		const result = await resp.text();  //isOk값을 text로 리턴
		return result;
	} catch(error){
		console.log(error);
	}
...
document.addEventListener('click', (e)=>{
	...
	if(e.target.classList.contains('cmtModBtn')){
		...
		// 내 타켓을 기준으로 가장 가까운 div를 찾기 closest('div')
